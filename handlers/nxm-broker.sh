...
instance_link="$HOME/.local/share/steamplay-skyrim-tools/modorganizer2/${nexus_game_id:?}"
...
if [ "$process_search_status" == "0" ]; then
	...
	download_start_output=$(WINEESYNC=1 WINEFSYNC=1 protontricks-launch --appid "$game_appid" "$instance_dir/nxmhandler.exe" "$nxm_link")
